<script lang="ts">
	...
	const validateCFPCNPJ = (payload: any) => {
	...
	const monetaryConverter = (payload: any, locale = "pt-BR") => {
		try {
			const parse = new Intl.NumberFormat(locale, {
				style: "currency",
				currency: "BRL",
			}).format(payload);
	...
	const caculateInstallments = (payload: any, installment: any) => {
		try {
	...
	const csvParser = (payload: string) => {
		try {
	...
	const downloadJson = (content: any) => {
		try {
...
<section>
	<div>
		<div class="padding">
			...
			<input type="file" on:input={fileReader} />
		</div>
